var ShopExGoodsEditor = new Class({
    ...
    initEditorBody:function(){
        var _this=this;
        var gcatSelect=$('gEditor-GCat-category');
        var gtypeSelect=$('gEditor-GType-input');

        gcatSelect && gcatSelect.addEvent('click',function(e){
            this.store('templabel', this.getElement('.label').get('text'));
            this.store('tempvalue', $('gEditor-GCat-input').get('value'));
            gtypeSelect.store('tempvalue', gtypeSelect.get('value'));
        });
        gtypeSelect.addEvent('focus',function(){
           this.store('tempvalue',this.get('value'));
        });
        gtypeSelect.addEvent('change',function(e){
            var tmpTypeValue = this.retrieve('tempvalue');
            var templabel;
            if (gcatSelect) templabel = gcatSelect.retrieve('templabel');
            if(this.get('value')&&confirm(LANG_goodseditor['comfirm'])){
                ...
                if(templabel)
                gcatSelect.getElement('.label').set('text', templabel);
                $('gEditor-GCat-input').set('value', gcatSelect.retrieve('tempvalue'));
           }
        });
    },
    ...
    qiyemprice:function(e){
        ...
        dom.getElements('input').each(function(el){
            if(el.name == 'price[]')
                ...
                info['price']=el.value;
            else if(el.getAttribute('qylevel'))
                info['qylevel['+el.getAttribute('qylevel')+']']=el.value;
        });
        window.fbox = new Dialog('index.php?app=b2c&ctl=admin_goods_editor&act=set_qiyemprice',{title:LANG_goodseditor['editvipprice'], ajaxoptions:{data:info,method:'post'},modal:true});
        window.fbox.onSelect = goodsEditor.setqiyeMprice.bind({base:goodsEditor,'el':dom});
    },
    setqiyeMprice:function(arr){
        var parr={};
        arr.each(function(p){
            parr[p.name] = p.value;});
        this.el.getElements('input').each(function(d){
            var level = d.getAttribute('qylevel');
			//alert(parr[level]);
