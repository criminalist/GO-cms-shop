...
$(document.html).addEvents({
    'click:relay(form .auto-change-verify-handle)': function (e,el) {//= 自动绑定更换验证码
        //点击更换验证码
        e.stop();
        var url;
        var img;
    ...
    'click:relay(form .auto-password-check-handle)': function (e,el) {
       passwordStrength.init(el);
    },
    'change:relay(form .action-remember-account)': function (e, el){
        if (el.checked) {
    ...
    'change:relay(form .action-auto-signin)': function (e, el){
        if (el.checked) {
...
(function() {
    ...
    this.Sync = new Class({
        ...
        success: function(text, xml) {
            ...
            if ((/text\/jcmd/).test(this.getHeader('Content-type')) || (/application\/json/).test(this.getHeader('Content-type'))) return this._jsonSuccess(text);
        ...
        _progress: function (rs) { //处理json数据
            that = this.sponsor;
            that.disabled = false;
            if (!rs) return;
            ...
            var msg;
            rs[rs.code == 0?'success':'error'] = rs.msg; //通过code来判断是否成功
            //every方法检测数组中的每个值在every函数里是否都为true
