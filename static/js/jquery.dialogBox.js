 ...
 ;(function($,window,document){
    ...
    var pluginName = 'dialogBox',
        defaults = {
            ...
            maskClickHide: false, //点击遮罩层关闭对话框
 		...
 		hide: function(element){
            var that = this;
 			...
 			setTimeout(function(){
				...
				if($.isFunction(that.settings.finish)){
					that.settings.finish();
                }
                if (that.settings.removeElement) {
                    $('#' + $this.attr('id')).remove();
                }
 			},150)
 		},
    ...
    $.extend({
        Dialog: function (options){
            var ident = 'showMsg' + parseInt(1000000 * Math.random());
            $('body').append('<div id="' + ident + '"></div>');
            options['removeElement'] = true; 
            new DialogBox($("#" + ident), options);
        }
    });
